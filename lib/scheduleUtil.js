    ...
    UserList.findOne({name:"expectedList"}).populate({path:"list"})
        .then((expectedList) => {
            for(i = 0; i < expectedList.list.length; i++){
                if(expectedList.list[i].state == "clear"){
...
// assumes the target user is "active"
async function setOneExpected(userid){
    var today = new Date().getDay();
    var expectedList = await UserList.findOne({name:"expectedList"});
    const week = ["Su","M","T","W","Th","F","S"];
    var dayCode = week[today];
    
    User.findOne({_id: userid})
        .then((user)=> { 
            var userindex = expectedList.list.indexOf(user._id);
            
            if(user.Schedule[dayCode]){
                user.state = "expected";
                if( userindex == -1){
                    expectedList.list.push(user._id);         // If the user isn't already on the expected list, add them.
                    expectedList.save();
                }
            }
                    ...
                    user.state = "idle";
                    if(userindex != -1){
                    expectedList.list.splice(userindex,1);         // If the user isn't already on the expected list, add them.
                    expectedList.save();
                    }
            }
            user.save();
